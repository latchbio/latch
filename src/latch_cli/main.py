...
def dockerfile(
    ...
    config = get_or_create_workflow_config(pkg_root=pkg_root, base_image_type=base_image)
...
def nf_generate_entrypoint(
    ...
    if metadata._nextflow_metadata is None:
        click.secho(
            dedent(f"""\
            Failed to generate Nextflow entrypoint. Make sure the project root contains a `{meta}` with a `NextflowMetadata` object defined.
            """),
            fg="red",
        )
        raise click.exceptions.Exit(1)
