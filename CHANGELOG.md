...
* `latch register` now accepts a `--dockerfile` parameter which allows a user to specify which Dockerfile to use for building their workflow image.
* `latch nextflow generate-entrypoint` now accepts a `--destination` flag which allows a user to specify where they would like to generate their entrypoint code.

### Changed

* `latch generate-metadata --nextflow` behavior has been modified in the following ways
  * It now writes autogenerated parameter metadata into a `generated.py` file.
  * It now natively creates dataclass and Enum definitions as necessary
  * It now generates a single dataclass containing all parameters (so the resulting entrypoint does not have to be updated if a parameter is added or removed)
  * It now automatically generates a base flow for the workflow.

### Dependencies

* Upgrade `lytekit` to version `0.15.29`
